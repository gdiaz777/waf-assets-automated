...
async function createAsset(assetData: any) {
    ...
    const { ASSET_DOMAIN, ASSET_NAME, ASSET_CERTIFICATE_TYPE, ASSET_HOST, ASSET_UPSTREAM, ASSET_CERT_KEY , ASSET_CERT_PEM } = assetData;
    ...
    if (ASSET_CERTIFICATE_TYPE === true) {
        ...
        const publicKey = await getEncryptionPublicKey(thisprofilepid, thisregion)
        const keyFile = ASSET_CERT_KEY
        const key = await Deno.readTextFile(keyFile)
        const certFile = ASSET_CERT_PEM 
        ...
        const profile = await getProfile(thisprofilepid)
        ...
        for (const thisdomain of ASSET_DOMAIN) {
                 // Find the matching domain in updatedCertificateDomainList
                const certificateDomainParematerIdNAME = updatedCertificateDomainList.find((item: any) => item.domain === thisdomain);
                ...
                if (certificateDomainParematerId) {
                    console.log("Uploading certificate for domain:", thisdomain);
...
// Get assets urls by ID
async function getWebApplicationAssetURLIdsById(assetId: string): Promise<string[] | null> {
    ...
    const urlBody = {
        "operationName": "WebApplicationAsset",
        "variables": {
            "id": assetId,
        },
        "query": "query WebApplicationAsset($id: ID!) { \n getWebApplicationAsset(id: $id) { \n URLs { \n URL \n } \n } \n }"
    };
    ...
    try {
        ...
        if (urlResponse.ok) {
                ...
                return urlData?.data?.getWebApplicationAsset?.URLs?.map((url: any) => url.URL) || [];
        } else {
                console.error("Failed to fetch URLs for the asset.");
                const errorData = await urlResponse.json();
                console.error("Error data:", errorData);
        }
    } catch (error) {
        console.error("Error fetching WebApplicationAsset:", error);
    }
    return null;
}

// Add new URL to existing asset
async function updateWebApplicationAsset(assetId: string, addURLs: string[]): Promise<any> {
    const url = "https://cloudinfra-gw.portal.checkpoint.com/app/waf//graphql";
    ...
    const body = {
        "operationName": "updateWebApplicationAsset",
        "variables": {
            "assetInput": {
                "addURLs": addURLs,
            },
        ...
        "query": `mutation updateWebApplicationAsset($assetInput: WebApplicationAssetUpdateInput!, $id: ID!) {
    ...
    try {
        ...
        if (response.ok) {
            console.log("WebApplicationAsset updated successfully");
            const concatenatedResults = await performConcatenatedActions();
            console.log("Concatenated Results:", JSON.stringify(concatenatedResults, null, 2));
            const data = await response.json();
            await publishAndEnforce();
            
            return data?.data?.updateWebApplicationAsset;
        } else {
            console.error("Failed to update WebApplicationAsset");
            const errorData = await response.json();
            console.error("Error data:", errorData);
        }
    } catch (error) {
        console.error("Error updating WebApplicationAsset:", error);
    }

    return null;
}

// Perform concatenated actions to update the existing asset
async function performConcatenatedActions(): Promise<any> {
    const url = "https://cloudinfra-gw.portal.checkpoint.com/app/waf//graphql";
    const token = wafSession?.data?.token; // Ensure wafSession is initialized
    const headers = {
        "Authorization": `Bearer ${token}`,
    ...
    const queries = [
        ...
        {
            operationName: "UndoChanges",
            query: `query UndoChanges {
                getUndoChanges {
                    action
                    timestamp
                    __typename
        ...
        {
            operationName: "RedoChanges",
            query: `query RedoChanges {
                getRedoChanges {
                    action
                    timestamp
                    __typename
        ...
        {
            operationName: "sessionStatus",
            query: `query sessionStatus {
                sessionStatus {
                    numberOfChanges
                    sessionDescription
                    __typename
    ...
    for (const query of queries) {
        ...
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                results[query.operationName] = data?.data;
            } else {
                console.error(`Failed to fetch ${query.operationName}`);
                const errorData = await response.json();
                console.error("Error data:", errorData);
                results[query.operationName] = null;
            }
        } catch (error) {
            console.error(`Error fetching ${query.operationName}:`, error);
            results[query.operationName] = null;
        }
    }
...
async function main() {
    ...
    FILEPATH = `./${formattedDate}_output.txt`; // File path with the formatted date
